...
from services.CreditCardService import analyze_credit_card

def configure_interface():
    st.title('Uplod de Arquivos')
    ...
    if uploaded_file is not None:
        ...
        if blob_url:
            st.write(f'Arquivo {fileName} enviado com suceso para o Azure Blob Storage')
            credit_card_info = analyze_credit_card(blob_url) # implementar funcao de cartao de credito
            show_image_and_validation(blob_url, credit_card_info) # type: ignore
        else:
